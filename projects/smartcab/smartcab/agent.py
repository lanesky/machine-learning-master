...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        maxQ = max(self.Q[state].iteritems(), key=(lambda x: x[1]))[1]
        #maxQ = [k for k,v in self.Q[state].items() if v == max_Q_value]
        return maxQ 
    ...
    def createQ(self, state):
        ...
        if self.learning and not self.Q.has_key(state):
            self.Q[state] = dict((k,0.0) for k in self.env.valid_actions)
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if self.epsilon ==0 or random.random() > self.epsilon:
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            ...
            self.Q[state][action] = prediction * (1.0 - self.alpha) + (reward * self.alpha)
        return
...
def run():
    ...
    #sim.run(n_test = 10)
    sim.run(n_test = 150,tolerance = 0.05)
